...
export type ExecutionMode = 'single_step' | 'navigation_plan';
...
export type ChallengeId = 'bench' | 'park' | 'tidy' | 'sort' | 'recharge' | 'apartment' | 'kitchen_bathroom';
...
export type ExchangeEntry = {
  id: number; title: string; timestamp: number; turn: number; image_url: string | null;
} & (
  ...
  { kind: 'feedback'; payload: { observation: Observation; image_detail: string; history_turns: number[]; input_items: number; images_in_request: number; tool_result_call_ids: string[]; context_mode?: string; memory_frame_seq?: number | null; recent_actions?: Partial<AgentAction>[]; collision_feedback?: CollisionFeedback | null } } |
