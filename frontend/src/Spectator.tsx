...
export function Spectator({ state, axes, enabled, onPlace }: { state: LiveState; axes: boolean; enabled: boolean; onPlace: (placement: ManualPlacement) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    camera.position.set(...(apartment ? [.45, 7.8, 2.8] : recharging ? [2, 2.6, 2.3] : parking ? [1.8, 1.5, 2] : [1.25, 1.05, 1.5]) as [number, number, number]);
    ...
    controls.target.set(apartment ? .45 : recharging ? .65 : parking ? .6 : .12, .25, 0);
    controls.enableDamping = true;
    controls.minDistance = .45;
    controls.maxDistance = apartment ? 12 : 8;
