...
**Past test results:** click **Test results** in the workspace navigation to open a separate read-only window. Search saved evaluation batches, filter by scenario/evidence, inspect trial outcomes, times, contacts, tokens and final camera images, or download a normalized JSON summary. Each recorded trial also shows its world-XY movement map with start/end/contact markers, time scrubbing and playback. **Top / 3D / Path** switches between the environment overlay and the plain route; the framing icon fits the route or whole room. The 3D view supports rotation, pan and zoom; hover or tap an object for its name. New recordings save the initial scene geometry and transforms. Older runs use a clearly labelled current-environment reconstruction, whose historical accuracy is unverified. Props remain at initial positions and textures come from the current app; this is not a dynamic scene replay. Manual relocations break the path; missing recordings remain unavailable. The camera is the final image, not a synchronized replay. Legacy and incomplete reports are labelled explicitly. Opening or closing this window does not connect to robot control, pause a run or reset the scene. It lists saved challenge evaluations under `.runtime`, including older report formats; live sessions and pytest/Playwright logs are not automatically included. Browser settings may open the separate window as a tab.
