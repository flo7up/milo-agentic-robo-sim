...
**Unified cockpit controls:** one sticky command bar contains power, Start, Stop, primary robot status, reported tokens / threshold and request count. Its options menu contains Reset episode and Enable manual control / Take manual control. Expanded camera, map and challenge dialogs use that same bar. The mission panel retains the input/output breakdown, goal, connection and run settings.

**Compact Luna contract (candidate):** `semantic-mission-v1` sends task-specific instructions/tools and a compact current-state summary, separating recent failures and excluding earlier missions' tasks. Luna can select one of up to four observed frontier IDs; the worker validates fresh stopped evidence and executes the existing bounded single-frontier capability. The observed-map trail now samples worker odometry between reviews and labels truncation. Offline replay reduced current-message JSON bytes by 77.7%; provider-token savings and real-model behavior remain unmeasured. Final fixed validation was **17 pass / 1 Home-return buffer-expiry failure / 3 prerequisite blocks**, so activation remains pending; the live backend was not restarted. [Contract and safety limits](docs/HOME_MAPPING.md#compact-semantic-mission-contract), [complete evidence](docs/DESIGN_PERFORMANCE.md).

**Run settings** now include **Luna request limit** (browser default 12) and **Luna token threshold** (default 100,000 input + output tokens). Redirecting a running task retains its accumulated usage. The request limit is checked before each request; the token threshold uses provider-reported usage after a response and may be exceeded by that response. Missing usage stops further inference, and requests already submitted before cancellation may still be billed. Plain exploration can execute locally after its initial approved plan; semantic object/room decisions still consult Luna. This does not fix model timeouts/403 errors or active sensor latency, and no real-model cost reduction has been measured. [Design, verification and limitations](docs/DESIGN_PERFORMANCE.md#current-answer).

**Saved workspace settings:** run settings, selected tabs and panels, per-scenario goal drafts, manual input values and challenge-picker choices now survive reloads and local preview-port changes. The last successfully loaded scene, including Practice Bench, restores on server startup as a fresh stopped episode; it never resumes motion or inference. Connection text is retained without credentials and still requires Apply when it differs from the running configuration. Preferences use the local workspace SQLite store, with browser recovery for unsaved edits. [Behavior, tests and limitations](.runtime/preferences-v1-review/summary.md).
...
**Robot observatory:** select a **Scenario** and **Load challenge** at the top; **Goal & objectives** expands its details. **Luna connection** opens automatically when configuration is missing. Once configured, the goal and Start action lead the flow, with controller options under **Run settings**. On mobile, setup appears before the views. After a run starts, **Conversation**, **Trace** and **Settings** provide run instructions, paired inputs/results and configuration; drafts survive tab switches. Manual controls are collapsed until **Controls** is selected, and the timeline appears after a manual action. Map destination selection appears after a scan, while busy states show progress and Stop instead of idle actions. SmolVLA controls appear only when selected or resident. Stop remains in the top toolbar throughout. The world view is operator-only; displayed decision reasons come from model responses, not reconstructed private reasoning.
...
**Run chat:** while either Luna navigation mode is active, send a **New instruction** in **Run chat**. This replaces the active goal, briefly stops the robot, discards pending actions, and replans in the same scene. The previous instructions remain historical context; include any constraints you still want enforced. No scene reset or SmolVLA weight reload is required. The run's remaining decision/time budget is preserved. Exact `stop`, `pause`, `halt`, or `cancel` messages stop locally without another model call; the main Stop button remains available during a pending chat update. Acknowledgements from **Controller** are separate from model-generated **Luna** reports.
