...
def test_presets_have_distinct_goals_and_private_geometry():
    assert set(PRESETS) == {"park", "tidy", "sort", "recharge", "apartment", "kitchen_bathroom"}
...
@pytest.mark.parametrize("identifier", ["sort", "kitchen_bathroom"])
...
@pytest.mark.parametrize("identifier", ["tidy", "kitchen_bathroom"])
...
async def test_charging_memory_keeps_initial_observation_without_target_coordinates():
    ...
    worker = SimulationWorker(challenge=preset, pace=False)
    model = ScriptedModel([model_response("observe", "{}", f"memory-{index}") for index in range(7)] +
                          [model_response("stop", "{}", "memory-stop")])
    agent = controller_for(model)
    try:
        await asyncio.wrap_future(worker.ready)
        agent.start(worker, AgentStart(run_id=worker.latest["run_id"], episode_epoch=worker.epoch,
                                     goal=preset.goal, feedback_interval_s=.25, max_turns=8))
        await agent.task
        ...
        assert sum(item.get("role") == "user" for item in last) == 8
